...
// 
// $Header: /cvsroot/acdk/acdk/acdk_core/tests/acdk/util/acdk_util_TContainer_Test.cpp,v 1.16 2005/02/05 10:45:09 kommer Exp $
...
#include "../../../src/acdk/util/THashMap.h"
#include "../../../src/acdk/util/TTreeMap.h"
#include "../../../src/acdk/util/TArrayList.h"
#include <acdk/util/TreeSet.h>

#include <acdk/tools/aunit/core_test.h>
#include <acdk/tools/aunit/TestRunner.h>

#if !defined(__GNUC__) || (__GNUC__ > 2) // only gcc 3.x can use the templates
...
BEGIN_DECLARE_TEST( TContainer_Test )
  DECLARE_TEST( standardMap )
  DECLARE_TEST( tArrayList )
END_DECLARE_TEST( TContainer_Test  )

BEGIN_DEFINE_TEST( TContainer_Test )
  ADD_TEST( TContainer_Test, standardMap ) 
  ADD_TEST( TContainer_Test, tArrayList ) 
  
END_DEFINE_TEST( TContainer_Test )
...
{
   {
    StringToStringTMap::RKeySetType keyset = map->keySet();
    testAssert(keyset->size() == map->size());
    StringToStringTMap::KeySetType::RIteratorType it = keyset->iterator();
    while (it->hasNext() == true)
    {
      RString str = it->next();
      if (map->containsKey(str) != true)
        testAssert(map->containsKey(str) == true);
    }
  }
  {
    StringToStringTMap::RValueCollectionType valcol = map->values();
    testAssert(valcol->size() == map->size());
    StringToStringTHashMap::ValueCollectionType::RIteratorType it = valcol->iterator();
    int ic = 0;
    while (it->hasNext() == true)
    {
      RString obj = it->next();
      if (map->containsValue(obj) != true)
...
void fetchElements(IN(RStringToStringTMap) map, IN(RStringArrayArray)  np)
{
  ...
  for (i = 0; i < size; ++i)
  {
    ...
    if (map->containsValue(&np[i][1]) != true)
...
{
  fillMap((RStringToStringTMap)smap, np);
  RStringToStringTSortedMap submap = smap->tailMap(&np[0][0]);
...
void testMap()
{
  ...
  //new THashMap<RString, RString>(16, 16);
  RStringToStringTHashMap hm = new StringToStringTHashMap();
  testContainer((RStringToStringTMap)hm, philophers_names);
  testContainer((RStringToStringTMap)hm, componist_names);
  RStringToStringTTreeMap tm = new StringToStringTTreeMap();

  //testContainer(&tm, philophers_names);
  testContainer((RStringToStringTMap)&tm, componist_names);
...
template <class T> 
void testTList(IN(RefHolder<TList<T> >) list, IN(RObjectArrayImpl<T>) ins)
{
  typedef T RValueType;

  typedef ::acdk::util::TList<RValueType> ListType;
  typedef typename ListType::RefType RListType;
  
  typedef ::acdk::util::TArrayList<RValueType> ArrayListType;
  typedef typename ArrayListType::RefType RArrayListType;
  
  typedef ::acdk::util::TIterator<RValueType> IteratorType;
  typedef typename IteratorType::RefType RIteratorType;

  typedef ::acdk::util::TListIterator<RValueType> ListIteratorType;
  typedef typename ListIteratorType::RefType RListIteratorType;

  //typedef ::acdk::lang::RObjectArray<T> RArrayType;

  int i;
  for (i = 0; i < ins->length(); ++i)
    list->add(ins[i]);
  testAssert(list->size() == ins->length());
  testAssert(list->remove(ins[2]) == true);
  
  testAssert(list->size() == ins->length() - 1);
  T t = list->remove(1);
  testAssert(list->size() == ins->length() - 2);

  int olenght = list->size();

  RefHolder< ::acdk::util::TArrayList<T> >  nl = new ::acdk::util::TArrayList<T>();
  for (i = 0; i < ins->length(); ++i)
    nl->add(ins[i]);
  list->addAll(&nl);
  testAssert(list->size() == olenght + ins->length());
  
  RIteratorType it = list->iterator();
  while (it->hasNext() == true)
  {
    RValueType t = it->next();
  }
  olenght = list->size();

  RListIteratorType lit = list->listIterator();
  while (lit->hasNext() == true)
  {
    RValueType t = lit->next();
    lit->remove();
    lit->add(t);
  }
  testAssert(list->size() == olenght);
  lit = list->listIterator(list->size() - 1);
  while (lit->hasPrevious() == true)
  {
    RValueType t = lit->previous();
  }
  list->clear();
  testAssert(list->size() == 0);
...
void TContainer_Test::standardMap()
{
  ...
  System::out->println(RString("TContainer::testMap takes millis: ") + millis);
  //testList();
  //testSet();
}

void
TContainer_Test::tArrayList()
{
  {
    USE_TARRAYLIST(String);
    USE_TLIST(String);
    RStringArrayList la = new StringArrayList();
    ...
    sa->append("list");
    sa->append("test");
    testTList<RString>((RStringList)(RObject)&la, sa);
  }
}
