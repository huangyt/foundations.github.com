...
if test 1 = 1; then
 ...
 $PERL ./genweb/bin/backup.pl \
  ...
  -P^cfg +P^cfg/general \
  -nimages +P^images \
  --name $NAME-$PACKAGEVERSION-src $NAME-$DISTRIBVERSION 

 rm ./$NAME-$DISTRIBVERSION
 cp $NAME-$PACKAGEVERSION-src.tgz $RPMHOME/SOURCES/$NAME-$PACKAGEVERSION-src.tar.gz
fi

rpm -vv -ba acdk/distrib/acdk_tools.spec
