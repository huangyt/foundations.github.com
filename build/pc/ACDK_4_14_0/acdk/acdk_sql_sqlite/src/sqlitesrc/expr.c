...
**
** $Id: expr.c,v 1.4 2005/04/05 12:30:25 kommer Exp $
*/
#include "sqliteInt.h"
#include <ctype.h>
...
ExprList *sqlite3ExprListDup(ExprList *p){
  ...
  pNew->a = pItem = (struct ExprList_item*)sqliteMalloc( p->nExpr*sizeof(p->a[0]) );
  if( pItem==0 ){
    sqliteFree(pNew);
    return 0;
  } 
...
struct SrcList *sqlite3SrcListDup(SrcList *p){
  SrcList *pNew;
  int i;
  int nByte;
  if( p==0 ) return 0;
...
IdList *sqlite3IdListDup(IdList *p){
  ...
  pNew->a = (struct IdList_item*)sqliteMallocRaw( p->nId*sizeof(p->a[0]) );
  if( pNew->a==0 ){
    sqliteFree(pNew);
    return 0;
  }
...
struct Select *sqlite3SelectDup(Select *p){
  Select *pNew;
  if( p==0 ) return 0;
...
ExprList *sqlite3ExprListAppend(ExprList *pList, Expr *pExpr, Token *pName){
  ...
  if( pList->nAlloc<=pList->nExpr ){
    ...
    a = (struct ExprList_item*)sqliteRealloc(pList->a, n*sizeof(pList->a[0]));
    if( a==0 ){
      goto no_mem;
    }
    pList->a = a;
...
static void getFunctionName(Expr *pExpr, const char **pzName, int *pnName){
  switch( pExpr->op ){
    case TK_FUNCTION: {
      *pzName = (const char*)pExpr->token.z;
...
void sqlite3ExprCode(Parse *pParse, Expr *pExpr){
  ...
  switch( op ){
    ...
    case TK_BLOB: {
      ...
      sqlite3VdbeOp3(v, op, 0, 0, (const char*)pExpr->token.z+1, pExpr->token.n-1);
      sqlite3VdbeDequoteP3(v, -1);
      break;
    }
#endif
