...
**
** $Id: alter.c,v 1.2 2005/04/04 23:26:29 kommer Exp $
*/
#include "sqliteInt.h"
#include <ctype.h>
...
static void renameTableFunc(
  ...
  int token;
  Token tname;
  char const *zCsr = (char const *)zSql;
  int len = 0;
  char *zRet;
  ...
  */
  if( zSql ){
    do {
      ...
      tname.z = (const unsigned char*)zCsr;
      tname.n = len;
      ...
      */
      do {
        zCsr += len;
        len = sqlite3GetToken((const unsigned char*)zCsr, &token);
...
static void renameTriggerFunc(
  ...
  Token tname;
  int dist = 3;
  char const *zCsr = (const char*)zSql;
  int len = 0;
  char *zRet;
