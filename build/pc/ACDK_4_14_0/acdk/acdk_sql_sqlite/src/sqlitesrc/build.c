...
**     ROLLBACK
**
** $Id: build.c,v 1.3 2005/04/04 23:33:12 kommer Exp $
*/
#include "sqliteInt.h"
#include <ctype.h>
...
static int findDb(sqlite3 *db, Token *pName){
  ...
  if( zName ){
    ...
    for(i=(db->nDb-1), pDb=&db->aDb[i]; i>=0; i--, pDb--){
      if( (!OMIT_TEMPDB || i!=1 ) && n==strlen((char*)pDb->zName) && 
          0==sqlite3StrICmp((const char*)pDb->zName, zName) ){
        break;
      }
    }
    sqliteFree(zName);
...
IdList *sqlite3IdListAppend(IdList *pList, Token *pToken){
  ...
  if( pList->nId>=pList->nAlloc ){
    ...
    a = (struct IdList_item*)sqliteRealloc(pList->a, pList->nAlloc*sizeof(pList->a[0]) );
    if( a==0 ){
