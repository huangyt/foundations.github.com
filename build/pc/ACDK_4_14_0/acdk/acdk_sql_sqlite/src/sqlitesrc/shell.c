...
**
** $Id: shell.c,v 1.2 2005/04/04 23:26:30 kommer Exp $
*/
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
...
static void output_csv(struct callback_data *p, const char *z, int bSep){
  ...
  }else if( isNumber((const unsigned char*)z, 0) ){
    fprintf(p->out,"%s",z);
  }else{
  ...
  switch( p->mode ){
    ...
    case MODE_Insert: {
      ...
      for(i=0; i<nArg; i++){
        ...
        }else if( isNumber((const unsigned char*)azArg[i], 0) ){
...
static int dump_callback(void *pArg, int nArg, char **azArg, char **azCol){
  ...
  if( strcmp(zType, "table")==0 ){
    ...
    while( rc==SQLITE_ROW ){
      ...
      zSelect = appendText(zSelect, (const char*)sqlite3_column_text(pTableInfo, 1), '"');
      rc = sqlite3_step(pTableInfo);
      if( rc==SQLITE_ROW ){
