...
**
** $Id: select.c,v 1.2 2005/04/04 23:26:30 kommer Exp $
*/
#include "sqliteInt.h"
  ...
  for(i=0; i<3 && apAll[i]; i++){
    ...
    for(j=0; j<sizeof(keywords)/sizeof(keywords[0]); j++){
      if( p->n==keywords[j].nChar 
          && sqlite3StrNICmp((const char*)p->z, (const char*)keywords[j].zKeyword, p->n)==0 ){
...
static void generateSortTail(
  ...
  if( pInfo==0 ) return;
  pInfo->aSortOrder = (u8*)&pInfo->aColl[nCol]; /* rrk is this correct? */
  pInfo->nField = nCol;
  for(i=0; i<nCol; i++){
