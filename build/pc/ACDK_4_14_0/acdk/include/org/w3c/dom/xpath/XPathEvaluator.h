...
public: 
  ...
  virtual RXPathNSResolver createNSResolver(IN(RNode) nodeResolver) = 0;
     ...
     */
  virtual RObject evaluate(IN(RString) expression, IN(RNode) contextNode, IN(RXPathNSResolver) resolver, short type,  IN(RObject) result) THROWS2(RXPathException, RDOMException) = 0;
};
