                           ...
                           void *value)
{
    ...
    facet->value = val;
    ret = (int)xmlSchemaCheckFacet(facet, typ, NULL, type);
    if (ret != 0) {
        xmlSchemaFreeFacet(facet);
        return (-1);
    }
    ret = (int)xmlSchemaValidateFacet((xmlSchemaTypePtr)typ, (xmlSchemaFacet*)facet, strval, (xmlSchemaVal*)value);
    xmlSchemaFreeFacet(facet);
    if (ret != 0)
        return (-1);
    return (0);
    ...
    while (cur != NULL) {
        if (((eora == 0) && ((cur->type == XML_RELAXNG_ELEMENT) ||
	        ...
	        xmlRelaxNGDefinePtr *temp;
                ...
                max *= 2;
                temp = (xmlRelaxNGDefinePtr*)xmlRealloc(ret,
...
xmlRelaxNGParseInclude(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node)
{
    ...
    incl = (xmlRelaxNGInclude*)node->psvi;
    if (incl == NULL) {
                         ...
                         const xmlChar * name)
{
    ...
    if (grammar->defs != NULL) {
        def = (xmlRelaxNGDefinePtr)xmlHashLookup(grammar->defs, name);
        if (def != NULL) {
            cur = ref;
            while (cur != NULL) {
                cur->content = def;
