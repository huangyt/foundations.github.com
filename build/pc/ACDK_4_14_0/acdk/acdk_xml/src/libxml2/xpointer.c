...
xmlXPtrRangeCheckOrder(xmlXPathObjectPtr range) {
    ...
    if (range->user2 == NULL)
	return;
    tmp = xmlXPtrCmpPoints((xmlNode*)range->user, range->index,
	                     (xmlNode*)range->user2, range->index2);
    if (tmp == -1) {
...
xmlXPtrEvalFullXPtr(xmlXPathParserContextPtr ctxt, xmlChar *name) {
	...
	if (ctxt->value != NULL) {
		...
		case XPATH_LOCATIONSET: {
		    xmlLocationSetPtr loc = (xmlLocationSet*)ctxt->value->user;
...
xmlXPtrCoveringRange(xmlXPathParserContextPtr ctxt, xmlXPathObjectPtr loc) {
    ...
    switch (loc->type) {
        case XPATH_POINT:
	    return(xmlXPtrNewRange((xmlNode*)loc->user, loc->index,
			           (xmlNode*)loc->user, loc->index));
        case XPATH_RANGE:
	    if (loc->user2 != NULL) {
		return(xmlXPtrNewRange((xmlNode*)loc->user, loc->index,
			              (xmlNode*)loc->user2, loc->index2));
	    } else {
