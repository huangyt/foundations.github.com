...
#ifdef LIBXML_XPTR_ENABLED
	{
	    xmlLocationSetPtr loc = (xmlLocationSet*)val->user;
		...
		break;
            ...
            switch (test) {
                case NODE_TEST_NONE:
                    ...
                    STRANGE return(t);
                case NODE_TEST_TYPE:
                    if ((cur->type == (xmlElementType)type) ||
                        ((type == NODE_TYPE_NODE) &&
    ...
    switch (op->op) {
		...
		CHECK_ERROR0;
                ...
                if ((op->ch2 != -1) &&
                    ...
                    val = (xmlXPathObject*)comp->steps[comp->steps[op->ch2].ch2].value4;
    ...
    switch (op->op) {
		...
		xmlDocPtr oldDoc;
                ...
                if ((op->ch1 != -1) && (op->ch2 != -1) &&
                    ...
                    val = (xmlXPathObject*)comp->steps[op->ch2].value4;
                    if ((val != NULL) && (val->type == XPATH_NUMBER) &&
                        (val->floatval == 1.0)) {
        ...
        case XPATH_OP_RANGETO:{
			...
			if (res->type == XPATH_LOCATIONSET) {
			    ...
			    for (j=0; j<rloc->locNr; j++) {
				  ...
				  (xmlNode*)oldlocset->locTab[i]->user,
				  oldlocset->locTab[i]->index,
				  (xmlNode*)rloc->locTab[j]->user2,
