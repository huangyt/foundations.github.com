                 ...
                 const xmlChar * namespace)
{
    ...
    if ((name == NULL) || (schema == NULL))
        ...
        ret = (xmlSchemaElement*)xmlHashLookup2(schema->elemDecl, name, namespace);
        if ((ret != NULL) &&
                 ...
                 const xmlChar * namespace)
{
    ...
    if (name == NULL)
        return (NULL);
    if (schema != NULL) {
        ret = (xmlSchemaTypePtr)xmlHashLookup2(schema->typeDecl, name, namespace);
                 ...
                 const xmlChar * namespace)
{
    ...
    ret = (xmlSchemaAttributePtr)xmlHashLookup2(schema->attrDecl, name, namespace);
                 ...
                 const xmlChar * namespace)
{
    ...
    ret = (xmlSchemaAttributeGroup*)xmlHashLookup2(schema->attrgrpDecl, name, namespace);
                 ...
                 const xmlChar * namespace)
{
    ...
    ret = (xmlSchemaType*)xmlHashLookup2(schema->groupDecl, name, namespace);
    if ((ret != NULL) && (ret->flags & XML_SCHEMAS_TYPE_GLOBAL))
	return (ret);  
    else
	ret = NULL;
			   ...
			   xmlSchemaTypePtr item)
{
    ...
    ((xmlSchemaTypePtr *) ass->items)[ass->nbItems++] = (xmlSchemaTypePtr)(void *) item;
    return (0);
}
		 ...
		 xmlNodePtr node)
{
    ...
    if (val != 0) {	
	    ...
	    prev = (xmlSchemaTypePtr)xmlHashLookup2(schema->typeDecl, name, namespace);
	    if (prev == NULL) {
		xmlSchemaPErr(ctxt, (xmlNodePtr) ctxt->doc,
			  ...
			  int absolute)
{
	...
	*/
    ...
    import = (xmlSchemaImport*)xmlHashLookup(schema->schemasImports, ns);
    if (import != NULL) {	
	/*
                      ...
                      xmlNodePtr node)
{
    ...
    */
    ...
    include->doc = doc;
    include->next = (xmlSchemaInclude*)schema->includes;
				...
				int fireErrors)
{
    ...
    */
	...
	while (facetLink != NULL) {
	    ...
	    */	    
		...
		xmlSchemaVFacetErr(ctxt, (xmlParserErrors)ret, node, value, len,
	...
	if (ret >= 0) {
	    ...
	    */
		...
		if (fireErrors)
		    xmlSchemaVFacetErr(ctxt, (xmlParserErrors)ret, node,
	...
	*/
	...
	if (retFacet > 0) {
	    ...
	    if (fireErrors) {
		xmlSchemaVFacetErr(ctxt, (xmlParserErrors)ret, node, value, 0, type, facet, 
		    NULL, NULL, NULL, NULL);
	    }
	}
    }	    
    ...
    */
	...
	if (ret < 0) {
	    ...
	    */
		...
		if (fireErrors) 
		    xmlSchemaVSimpleTypeErr(ctxt, (xmlParserErrors)ret, node, value, type);
	    }	
	}	
...
{        
	...
	*/
	...
	if (node->ns != NULL)
	    decl = (xmlSchemaElementPtr)xmlHashLookup3(ctxt->schema->elemDecl,
	    node->name, node->ns->href, NULL);
	else 
	    decl = (xmlSchemaElementPtr)xmlHashLookup3(ctxt->schema->elemDecl, node->name, NULL, NULL);
	if (decl != NULL) {		    
	    ctxt->node = node;	
    ...
    */
	...
	if (cur->type == XML_ELEMENT_NODE) {
	    ...
	    */		
		...
		decl = (xmlSchemaElementPtr)xmlHashLookup3(ctxt->schema->elemDecl, cur->name, NULL, NULL);	    
	    if (decl != NULL) {		    
		ctxt->node = cur;
		ret = xmlSchemaValidateElementByDeclaration(ctxt, decl);
		ctxt->node = top;
